...
The following content from the original document was removed because it conflicts with Upsun Flex sizing/costing or is out of scope for this skill:

- All `size: AUTO`, `size: S/M/L/XL/2XL`, `base_memory`, `memory_ratio` references — Flex uses an explicit CPU matrix with profile-derived RAM[1].
- "No Horizontal Scaling / Grid projects support only 1 instance" — incorrect for Flex, which supports horizontal scaling of applications[1].
- All YAML configuration snippets (`applications.yaml`, `services.yaml`, `routes.yaml`, `hooks.yaml`, mounts, `variables.php`, ImageMagick `policy.xml`).
- All deployment-mechanics content: build/deploy/post_deploy hooks, `.platform-scripts/*`, environment variables, `PLATFORM_RELATIONSHIPS`, S3/CloudFront integration steps, custom-domain procedures, troubleshooting (SSH, logs).
- Console-variable lists (`PIMCORE_TOKEN`, install secrets, etc.) — operational, not sizing.
- Cron job command lists and parallelization flags — operational, not sizing (kept only the generic statement that workers and crons exist and need their own containers).
- Generic best-practice sections (security, dev workflow, branch strategy) unrelated to sizing/costing.
- External documentation links pointing to `fixed.docs.upsun.com` (the legacy Fixed product).